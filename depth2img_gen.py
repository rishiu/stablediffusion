...
def main():
    ...
    with open(opt.in_file, "r") as in_file:
    ...
    # key_list = list(prompts_data.keys())
    # midas_trafo = AddMiDaS(model_type="dpt_hybrid")

    # idx = 0
    # while idx < len(key_list):
    #     bs = 4
    #     if idx + 4 >= len(key_list):
    #         bs = len(key_list) - idx - 1
    #     imgs = []
    #     prompts = []
    #     vpts = []
    #     midas_in = []
    #     for i in range(bs):
    #         fname = key_list[idx+i]
    #         image = np.array(Image.open(opt.data_dir+fname).convert("RGB"))
    #         image = torch.from_numpy(image).to(dtype=torch.float32) / 127.5 - 1.

    #         txt = prompts_data[fname]
    #         im_vpts = vpts_data[fname]

    #         if len(im_vpts) < 3:
    #             new_vps = [[0.0,0.0]] * (3 - len(im_vpts))
    #             im_vpts += new_vps

    #         temp_batch = {
    #             "image": image
    #         }
    #         temp_batch = midas_trafo(temp_batch)
    #         imgs.append(image)
    #         prompts.append(txt)
    #         vpts.append(im_vpts)
    #         midas_in.append(torch.from_numpy(temp_batch["midas_in"]).to(device=device))
    #     imgs = torch.stack(imgs)
    #     midas_in = torch.stack(midas_in)
    ...
    #     batch["image"] = rearrange(batch["image"].to(device=device), 'n h w c -> n c h w')
    #     print(batch["vpts"])
    #     batch["vpts"] = torch.tensor(batch["vpts"], device=device)

    #     base_loss = get_samples(batch, base_model, base_sampler, seed=opt.seed, n_samples=opt.n_samples, scale=opt.scale, save_img=idx, name="base")
