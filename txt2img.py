...
def main():
    ...
    sampler.make_schedule(50, ddim_eta=0, verbose=True)
    #sampler2.make_schedule(50, ddim_eta=0, verbose=True)
    ...
    if opt.depth != "":
        depth_data = np.load(opt.depth,allow_pickle = True)
        #depth_data = np.load(opt.depth)
        #depth_data = depth_data["depth"][:,:,0]
        depth_data = torch.tensor(depth_data[None, None, :, :], device="cuda")

        dpt_data = depth_data.detach().cpu().numpy()
        im = plt.imshow(dpt_data[0,0,:,:])
        plt.colorbar(im)
        plt.savefig("test_depth2.jpg")
        plt.close()
        #dpt_model = torch.hub.load("intel-isl/MiDaS", "DPT_Large")
        #dpt_model.cuda()
        #dpt_model.eval()

        #tform = torch.hub.load("intel-isl/MiDaS", "transforms").dpt_transform

        #img = np.array(Image.open(opt.depth))
        #img = tform(img).cuda()

        #depth_data = dpt_model(img)

        depth_data = torch.nn.functional.interpolate(
            depth_data,
            size=start_code.shape[2:],
            mode="bilinear",
            align_corners=False
        )
        ...
        depth_min, depth_max = torch.amin(depth_data, dim=[1, 2, 3], keepdim=True), torch.amax(depth_data, dim=[1, 2, 3], keepdim=True)        #depth_data = torch.tile(depth_data, (4,1,1,1))
        #print(depth_min, depth_max)
        depth_data = 2. * (depth_data - depth_min) / (depth_max - depth_min) - 1.
        dpt_data = depth_data.detach().cpu().numpy()
        im = plt.imshow(dpt_data[0,0,:,:])
        plt.colorbar(im)
        plt.savefig("test_depth.jpg")
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for prompts in tqdm(data, desc="data"):
                        ...
                        if opt.depth != "":
                            ...
                            uncond = {"c_concat": [depth_data], "c_crossattn": [uc]}
                        else:
                            cond = c
                            uncond = uc
                        print(c.shape)
                        ...
                        #                                 x_T=start_code)
                        t_enc = 45#min(int(strength * steps), steps-1)
                        samples = sampler.decode(start_code, cond, t_enc, unconditional_guidance_scale=opt.scale, unconditional_conditioning=uncond)
            ...
            with model2.ema_scope():
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for prompts in tqdm(data, desc="data"):
                        ...
                        t_enc = 45#min(int(strength * steps), steps-1)
                        samples = sampler2.decode(start_code, cond, t_enc, unconditional_guidance_scale=opt.scale, unconditional_conditioning=uncond)
                        x_samples_ddim = model2.decode_first_stage(samples)
                        x_samples_ddim = torch.clamp((x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0)
